...
// Parse pointer for compilation and registration of instructions in memory
...
enum InstructionType {
    jump, add, sub, mul, beq, bne, slt, lw, sw, addi
};
...
std::map<InstructionType, int> op_codes({ 
    std::make_pair(add, 32), 
...
// Map between branch labels and addresses
...
int main(int argc, char* argv[]){
    ...
    if (!freopen(filename.c_str(), "r", stdin)){
        std::cerr << "Wrong file path given" << std::endl;
        exit(-1);
    }

    optimize = (argc == 2) && ((std::string) argv[1] == "1");

    DRAM::active_row = -1;
    DRAM::ROW_ACCESS_DELAY = 10;
    DRAM::COL_ACCESS_DELAY = 2;
...
void throwRunTimeError(const std::string& message, unsigned int current){
    std::cerr << "Runtime error at instruction number " << current << std::endl;
    std::cerr << message << std::endl;
    exit(-1);
}

void throwRunTimeError(const std::string& message){
    std::cerr << "Runtime error occured" << std::endl;
    std::cerr << message << std::endl;
    exit(-1);
}

// Functions to throw compilation error
void throwError(const std::string& message, int& lineNum, int& column){
    std::cerr << "Syntax error: At line " << lineNum << ", column " << column+1 << std::endl;
    std::cerr << message << std::endl;
    exit(-1);
}

void throwError(const std::string& message, int& lineNum){
    std::cerr << "Syntax error: At line " << lineNum << std::endl;
    std::cerr << message << std::endl;
    exit(-1);
}

void printOverFlowMessage(std::string& s, unsigned int& current){
    std::cerr << s << std::endl;
    std::cerr << "At instruction " << current << std::endl;
}

void detectOFadd(int a, int b){
    std::string msg = "Warning : Overflow detected in addition";
    if ((b > 0) && (a > INT32_MAX - b)){
        printOverFlowMessage(msg, current);
    }
...
InstructionType getInstructionType(unsigned int opcode, unsigned int& current){
    for (std::pair<InstructionType, int> temp: op_codes){
        if (temp.second == opcode){
            return temp.first;
        }
    }
    throwRunTimeError ("No such instruction", current);
    return lw;
}
...
std::vector<unsigned int> getParams(InstructionType i_type, unsigned int& instr){
    ...
    if (i_type == jump){
        ...
        throwRunTimeError("Invalid instruction", (unsigned int) 0);
    }
...
void execute(){
    ...
    if (branches.find("main") == branches.end()){
        throwRunTimeError("Address not found: main");
        return;
    }else{
    ...
    // end
        ...
        InstructionType i_type = getInstructionType(opcode, current);

        std::vector<unsigned int> params = getParams(i_type, instr);
    ...
    printRegisterFile();
    std::cout << std::endl << "Total row updates = " << row_updates_count << std::endl;
    // end

    std::cout << std::endl << "Total clock cycles: " << totalCycles << std::endl << std::endl;
...
std::vector<std::string> getLoadSaveTokens(std::string& s, int& i, int& lineNum){
    std::vector<std::string> res;
    int j = i;
    while (!isSpace(s.at(i)) && s.at(i) != ','){
        i++;
        if (i >= s.length()){
            throwError("Invaid input", lineNum);
        }
    }
    ...
    while (s.at(i) != '('){
        if (!isSpace(s.at(i))){
            throwError("Invaid input", lineNum, i);
        }
        i++;
        if (i >= s.length()){
            throwError("Invaid input", lineNum);
    ...
    toNextNonSpace(s, i);
    if (i >= s.length()){
        throwError("Invalid input", lineNum);
    }
    j = i;
    while (!isSpace(s.at(i)) && s.at(i) != ')'){
        i++;
    ...
    toNextNonSpace(s, i);
    if (s.at(i) != ')'){
        throwError("Invalid input: expected a \')\'", lineNum, i);
    }
    i++;
    toNextNonSpace(s, i);
    if (i < s.length()){
...
void parse(std::string s, int& lineNum){
    ...
    if (!isAlpha(s.at(i))){
        throwError("Expected an english alphabet", lineNum, i);
    }
    ...
    while (isAlpha(s.at(i)) || isNum(s.at(i))){
        i++;
        if (i == s.length()){
            throwError("Invalid input", lineNum, i);
        }
    }
    ...
    if (s.at(i) == ':'){
        if (branches.find(arg1) != branches.end()){
            throwError("Invalid input: Duplicate labels found", lineNum);
        }
        ...
        if (!isAlpha(s.at(i))){
            throwError("Expected an alphabet got a number!", lineNum, i);
        }
        int k = i++;
    ...
    if (i >= s.length()){
        throwError("Expecting an argument", lineNum);
    }

    std::vector<std::string> tokens;
    
    if (arg1 == "lw" || arg1 == "sw") {
        tokens = getLoadSaveTokens(s, i, lineNum);
    }else{
    ...
    if (arg1 == "j"){
        ...
        throwError("Invalid token found " + arg1, lineNum, i);
    }
...
void parseInstruction(InstructionType i_type, std::vector<int> params, int& lineNum){
    if (i_type == jump){
        ...
        if (params[2] > INT16_MAX || params[2] < INT16_MIN) throwError("Integer out of range", lineNum);
...
void parse3Arg(std::vector<std::string>& tokens, int& lineNum, InstructionType i_type){
    ...
    if (register_map.find(arg1) == register_map.end()){
        throwError("Invalid register", lineNum);
        return;
    }
...
void parseMul(std::vector<std::string>& tokens, int& lineNum){
    parse3Arg(tokens, lineNum, mul);
}

// Function to parse slt instruction
void parseSlt(std::vector<std::string>& tokens, int& lineNum){
    parse3Arg(tokens, lineNum, slt);
}
...
void parseAddi(std::vector<std::string>& tokens, int& lineNum){
    ...
    try{
        ...
        throwError ("Invalid integer argument", lineNum);
        return;
    }
...
void parseLw(std::vector<std::string>& tokens, int& lineNum){
    ...
    try{
        ...
        throwError("Invalid offset", lineNum);
    }
